...
impl Game {
    pub fn new(config: GameConfig) -> Self {
        Self {
            ...
            rate_limiter: RateLimiterImpl::new(config.tile_wait_time),
        }
    }
    ...
    pub fn load(board: impl Into<Vec<u8>>, height: GameConfig) -> Result<Game, LoadError> {
        ...
        Ok(Game {
            ...
            rate_limiter: RateLimiterImpl::new(height.tile_wait_time),
        })
    }
...
mod tests {
    ...
    mod load {
        ...
        #[rstest]
        #[tokio::test]
        async fn initially_loaded_board_must_exists_as_is(load_conf: GameConfig) {
        ...
        #[rstest]
        #[tokio::test]
        async fn it_must_return_error_if_width_and_height_do_not_match(load_conf: GameConfig) {
        ...
        #[rstest]
        #[tokio::test]
        async fn it_must_return_error_if_loaded_board_is_invalid(load_conf: GameConfig) {
    ...
    mod set_tile {
        ...
        async fn it_must_return_error_if_index_is_out_of_range(game: Game) {
            let err = game
                .set_tile(TEST_IP, WIDTH * HEIGHT, 0).await
        ...
        async fn it_must_return_error_if_color_is_not_valid(game: Game) {
            let err = game
                .set_tile(TEST_IP, 0, MAX_COLOR + 1).await
                .expect_err("expected set_tile to return error");
        ...
        mod rate_limit {
            ...
            #[rstest]
            #[tokio::test]
            async fn multiple_set_tiles_with_different_ips_should_not_rate_limit_each_other(game: Game) {
                game.set_tile(TEST_IP, 0, 0).await.unwrap();
                assert!(game
                    .set_tile(IpAddr::V4(std::net::Ipv4Addr::new(127, 0, 0, 2)), 0, 0).await
                    .is_ok());
            }
        }
